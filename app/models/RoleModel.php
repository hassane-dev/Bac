...
class RoleModel {
    ...
    public function delete($id) {
        // La table `roles_accreditations` a ON DELETE CASCADE pour role_id.
        // La table `users` a ON DELETE RESTRICT pour role_id.
        // Il faut donc s'assurer qu'aucun utilisateur n'est assigné à ce rôle avant de le supprimer.
        ...
        if ($userCount > 0) {
            // Ne pas supprimer si des utilisateurs sont assignés
    ...
    public function updateAccreditations($role_id, $accreditation_ids = []) {
        ...
        // Supprimer les anciennes accréditations pour ce rôle
        ...
        // Ajouter les nouvelles accréditations
        if (!empty($accreditation_ids)) {
            // S'assurer que les IDs sont des entiers pour la sécurité
            ...
            foreach ($accreditation_ids as $acc_id) {
                if ($acc_id > 0) { // S'assurer que l'ID est valide
                    $this->db->bind(':role_id', $role_id);
