...
// $title, $roles, $user (contient id, username etc.), $tr, et les variables _err sont passées par le contrôleur
?>

<div class="container mt-4">
    <h2><?php echo htmlspecialchars($title); ?>: <?php echo htmlspecialchars($username ?? ''); ?></h2>
    ...
    <form action="<?php echo $app_url; ?>/users/update/<?php echo htmlspecialchars($id ?? ''); ?>" method="POST" enctype="multipart/form-data">
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    <label for="mot_de_passe" class="form-label"><?php echo $tr('password'); ?> (<?php echo $tr('leave_blank_if_no_change'); // Laisser vide pour ne pas changer ?>)</label>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select <?php echo (!empty($role_id_err)) ? 'is-invalid' : ''; ?>" id="role_id" name="role_id" required <?php echo ($id == 1) ? 'disabled' : ''; // Empêcher le changement de rôle pour l'admin ID 1 ?>>
                    ...
                    <?php if ($id == 1): // Si admin ID 1, ajouter un champ caché pour que role_id soit quand même soumis ?>
                ...
                <div class="mb-3 form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                        ...
                        <?php echo ($id == 1 || (isset($_SESSION['user_id']) && $id == $_SESSION['user_id'])) ? 'disabled' : ''; // Empêcher désactivation admin ID 1 ou soi-même ?> >
                    <label class="form-check-label" for="is_active"><?php echo $tr('is_active_user'); ?></label>
                     <?php if ($id == 1 || (isset($_SESSION['user_id']) && $id == $_SESSION['user_id'])): ?>
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <?php if (!empty($photo)): ?>
                        <img src="<?php echo $app_url . '/' . htmlspecialchars($photo); // Assumer que photo est un chemin relatif depuis la racine de public ?>" alt="Photo actuelle" class="img-thumbnail mt-2" style="max-height: 100px;">
                    <?php endif; ?>
                </div>
                 */ ?>
            </div>
