        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="<?php echo APP_URL . '/users/edit/' . $data['id']; ?>" method="POST" novalidate>
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="role_id" id="role_id" class="form-select <?php echo (!empty($data['role_id_err'])) ? 'is-invalid' : ''; ?>" required
                                <?php if ($data['id'] == 1 && ($_SESSION['user_id'] ?? null) != 1) echo 'disabled'; // Admin ne peut changer son propre rôle que s'il est l'admin connecté  ?>
                                <?php if ($data['id'] == 1 && ($_SESSION['user_id'] ?? null) == 1 && $data['id'] == ($_SESSION['user_id'] ?? null) ) echo ''; // L'admin peut changer son propre rôle ?>
                                <?php if ($data['id'] != 1 && $data['id'] == ($_SESSION['user_id'] ?? null) ) echo 'disabled'; // User ne peut pas changer son propre rôle s'il n'est pas admin
                                ?>
                            >
                            ...
                            <?php if ($data['id'] == 1 && ($_SESSION['user_id'] ?? null) != 1): ?>
                                <input type="hidden" name="role_id" value="<?php echo $data['role_id']; // Conserver la valeur si désactivé ?>" />
                            <?php endif; ?>
                             <?php if ($data['id'] != 1 && $data['id'] == ($_SESSION['user_id'] ?? null) ): ?>
                        ...
                        <div class="col-md-6 mb-3 align-self-center">
                             <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1"
                                    ...
                                    <?php // Empêcher la désactivation du compte admin principal ou de son propre compte si on n'est pas un autre admin
                                        if ($data['id'] == 1) echo 'disabled onclick="return false;"';
                                        if ($data['id'] == ($_SESSION['user_id'] ?? null) && $data['id'] != 1) echo 'disabled onclick="return false;"';
                                    ?>
                                >
                                <label class="form-check-label" for="is_active">
                                    <?php echo $tr('is_active_user'); ?>
                                </label>
                                <?php if (($data['id'] == 1 || ($data['id'] == ($_SESSION['user_id'] ?? null) && $data['id'] != 1)) && $data['is_active']): ?>
                    ...
                    <!-- Champ pour upload de photo (à implémenter plus tard) -->
                    <!-- <div class="mb-3">
                        <label for="photo" class="form-label"><?php echo $tr('current_photo'); ?>: ...</label>
