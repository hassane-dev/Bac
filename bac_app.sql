...
CREATE TABLE IF NOT EXISTS `eleves` (
  ...
  `empreinte1` TEXT NULL, `empreinte2` TEXT NULL, `empreinte3` TEXT NULL, `empreinte4` TEXT NULL, `empreinte5` TEXT NULL,
  `empreinte6` TEXT NULL, `empreinte7` TEXT NULL, `empreinte8` TEXT NULL, `empreinte9` TEXT NULL, `empreinte10` TEXT NULL,
  `annee_scolaire_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `notes` (
  ...
  CONSTRAINT `chk_note_range` CHECK ((`note` between 0 and 20))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `templates_documents` (
  ...
  `element` VARCHAR(50) NOT NULL COMMENT 'Pour les éléments textuels/images. Pour le fond, utiliser une valeur conventionnelle ex: ''__DOCUMENT_BACKGROUND__''',
  ...
  `langue_affichage` ENUM('fr','ar','fr_ar') NULL DEFAULT 'fr_ar' COMMENT 'Langue pour cet élément spécifique; fr_ar pour bilingue si l élément le supporte. Peut être non pertinent pour le fond global.',
  `visible` BOOLEAN NOT NULL DEFAULT TRUE,
  `est_parametre_fond` BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'TRUE si cette ligne définit le fond du document, FALSE si c est un élément textuel/image superposé',
  `type_fond` ENUM('couleur', 'theme_app', 'image_upload') NULL COMMENT 'Applicable si est_parametre_fond=TRUE',
  `valeur_fond` VARCHAR(255) NULL COMMENT 'Code couleur HEX, nom du thème, ou chemin relatif de l image de fond (ex: backgrounds/diplome_bg.jpg). Applicable si est_parametre_fond=TRUE',
  `opacite_fond` FLOAT NULL DEFAULT 1.0 COMMENT 'Opacité pour le fond (image/couleur). Applicable si est_parametre_fond=TRUE. Doit être entre 0 et 1.',
  UNIQUE KEY `uk_template_item` (`type_document`, `element`, `langue_affichage`, `est_parametre_fond`),
  CONSTRAINT `chk_opacite_fond_range` CHECK ((`opacite_fond` >= 0 AND `opacite_fond` <= 1))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
VALUES (1, 'admin', '$2y$10$N.SitGu8P8zMvLp9Tj9xS.27Uvztg679yC2.xM5FzBHDBu62Y09zO', 1, 'Admin', 'Sys', '1990-01-01', 'System', 'M', TRUE);
...
-- Exemple d'une configuration de fond pour le diplôme
-- INSERT IGNORE INTO `templates_documents` (`type_document`, `element`, `est_parametre_fond`, `type_fond`, `valeur_fond`, `opacite_fond`, `langue_affichage`, `visible`)
-- VALUES ('diplome', '__DOCUMENT_BACKGROUND__', TRUE, 'couleur', '#EAEAEA', 1.0, 'fr', TRUE);
