    ...
    <form action="<?php echo $app_url; ?>/anneesscolaires/update/<?php echo htmlspecialchars($id ?? ''); ?>" method="POST">
        ...
        <div class="mb-3 form-check form-switch">
            <input class="form-check-input" type="checkbox" id="est_active" name="est_active" value="1"
                ...
                <?php echo (isset($est_active) && $est_active) ? 'disabled' : ''; // On ne peut pas désactiver directement l'année active ici, il faut en activer une autre depuis la liste ?>
            >
            <label class="form-check-label" for="est_active"><?php echo $tr('is_active_year_status'); ?></label>
            <?php if (isset($est_active) && $est_active): ?>
                <small class="form-text text-muted d-block"><?php echo $tr('uncheck_not_supported_activate_another'); ?></small>
