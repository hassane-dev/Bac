...
class User {
     ...
     * Utile pour la connexion et pour vérifier l'unicité.
     ...
     * @param int $id
     * @param array $data Données de l'utilisateur. Si mot_de_passe est fourni, il sera haché.
     * @return bool
     */
    public function update($id, $data) {
        // Construire la requête dynamiquement pour ne mettre à jour le mot de passe que s'il est fourni
    ...
    public function delete($id) {
        // Idéalement, ne pas permettre la suppression de l'utilisateur admin principal (ex: ID 1)
        if ($id == 1) { // Supposition
            return false;
        }
    ...
    public function emailExists($email, $currentId = null) {
        if (empty($email)) return false; // Ne pas vérifier si l'email est vide
